 ...
 * @config {Number} vars.gridWidth 每一个预览文化所占的宽度，应该为flash寛的整除
 * @config {Number} vars.gridHeight 每一个预览文化所占的高度，应该为flash高的整除
 * @config {Number} vars.picWidth 单张预览文化的宽度
 * @config {Number} vars.picHeight 单张预览文化的高度
 * @config {String} vars.uploadDataFieldName POST请求中文化数据的key,默认值'picdata'
 * @config {String} vars.picDescFieldName POST请求中文化描述的key,默认值'picDesc'
 * @config {Number} vars.maxSize 文件的最大体积,单位'MB'
 * @config {Number} vars.compressSize 上传前如果文化体积超过该值，会先压缩
 ...
 * @param {String} backgroundUrl 背景文化路径
 * @param {String} listBacgroundkUrl 布局控件背景
 * @param {String} buttonUrl 按钮文化不背景
